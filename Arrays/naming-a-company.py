""" [HARD]
You are given an array of strings ideas where each string 
represents a word. A valid name can be formed by swapping 
the first letter of two different words in the list. 
However, you can only count the name as valid if it does 
...
The 2 in combinations(wordMap, 2) specifies that you want to 
generate pairs (i.e., combinations of 2 elements).
 
Imports: The defaultdict and combinations are imported 
to simplify dictionary handling and pair generation.

Grouping Words: Words are grouped by their first character, 
storing the suffixes in a set for efficient lookup.

Counting Pairs: The combinations function generates pairs 
of different characters, ensuring that each pair is only considered once.

Calculating Valid Pairs: For each character pair, 
common suffixes are subtracted to find distinct suffixes, and the product of these distinct counts is added to the result.
